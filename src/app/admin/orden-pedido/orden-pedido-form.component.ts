...
export class OrdenPedidoFormComponent implements OnInit, OnDestroy {
    ...
    constructor(
        ...
        this.form = this.fb.group({
            opFecha: [new Date().toISOString().split('T')[0], Validators.required],
    ...
    calcularTotal(): void {
        ...
        this.form.get('opPrecioTotal')?.setValue(+(sup * precioHa).toFixed(2), { emitEvent: false });
    }

    populateForm(): void {
        this.form.patchValue({
            opFecha: this.data!.opFecha,
        ...
        for (let i = 1; i <= 2; i++) {
            ...
            if (nom) this.addCoadyuvante(nom);
        }
    }
    ...
    filterAgroquimicos(val: string, index: number): void {
        const filter = val.toLowerCase();
        this.filteredAgroquimicos = this.agroquimicos.filter(a =>
            (a.listadoAgroqNom || '').toLowerCase().includes(filter)
        );
    ...
    addCoadyuvante(nombre?: string): void {
        if (this.coadyuvantesArray.length >= 4) return;
        this.coadyuvantesArray.push(this.fb.group({ nombre: [nombre || '', Validators.required] }));
    }
    ...
    filterCoadyuvantes(val: string): void {
        const filter = val.toLowerCase();
        this.filteredCoadyuvantes = this.coadyuvantes.filter(c =>
            (c.ListadoCoadNom || '').toLowerCase().includes(filter)
        );
    }
    ...
    onSubmit(): void {
        ...
        this.coadyuvantesArray.controls.forEach((ctrl, i) => {
            const v = ctrl.value;
            if (v.nombre) orden[`opCoad${i + 1}`] = v.nombre;
        });
        ...
        req.pipe(takeUntil(this.unsubscribe$)).subscribe({
            next: () => {
                Swal.fire('Éxito', this.isEdit ? 'Orden actualizada' : 'Orden creada', 'success');
